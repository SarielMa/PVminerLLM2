#%%writefile /home/lm2445/project_pi_sjf37/lm2445/PV_multiagent/sft/prepare_targeted_preference_data_revised.py
#!/usr/bin/env python3
"""
REVISED: Prepare TARGETED Preference Dataset Using Actual Model Confusion Matrices
...
Usage:
    python prepare_targeted_preference_data_REVISED.py \
        --input_dir /path/to/arrow/dataset \
        --output_dir /path/to/preference/dataset \
...
def load_confusion_matrix(filepath: str) -> Dict[Tuple[str, str], int]:
    ...
    with open(filepath, 'r') as f:
        ...
        for row in reader:
            ...
            elif 'Ground_Truth_SubCode' in row or 'Ground_Truth_Subcode' in row:
                # Sub-code level confusion
                gt = row.get('Ground_Truth_SubCode', row.get('Ground_Truth_Subcode'))
                pred = row.get('Predicted_SubCode', row.get('Predicted_Subcode'))
            else:
                continue
...
def convert_to_preference_format(
    dataset: Dataset, 
    negative_generator: TargetedNegativeGenerator,
    negatives_per_sample: int = 1
) -> Dataset:
    """
    Convert standard dataset to preference format with targeted negatives
    ...
    print(f"\n✓ Generated {len(preference_examples)} preference pairs")
    ...
    for strategy, count in sorted(strategy_counts.items(), key=lambda x: x[1], reverse=True):
        pct = 100 * count / len(preference_examples) if len(preference_examples) > 0 else 0
...
def filter_identical_pairs(preference_dataset: Dataset) -> Dataset:
    """
    ...
    for sample in preference_dataset:
    ...
    if removed == 0:
        ...
        print(f"⚠️  Filtered {removed} identical pairs ({100*removed/len(preference_dataset):.1f}%)")
    
    print(f"✓ Kept {len(filtered)} valid pairs ({100*len(filtered)/len(preference_dataset):.1f}%)")
    print("="*80)
    
    return Dataset.from_list(filtered)


def verify_and_print_samples(preference_dataset: Dataset, n_samples: int = 3):
    """Print first N samples for manual inspection"""
    
    print("\n" + "="*80)
    print(f"SHOWING FIRST {n_samples} PREFERENCE SAMPLES")
    print("="*80)
    
    for i in range(min(n_samples, len(preference_dataset))):
        sample = preference_dataset[i]
        metadata = json.loads(sample["metadata"])
...
def main():
    parser = argparse.ArgumentParser(description="Prepare targeted preference dataset using confusion matrices (REVISED)")
    parser.add_argument("--input_dir", type=str, required=True,
                       help="Path to input Arrow dataset")
    parser.add_argument("--output_dir", type=str, required=True,
                       help="Path to save preference dataset")
    ...
    print("="*80)
    print("TARGETED PREFERENCE DATASET PREPARATION (REVISED)")
    ...
    preference_dataset = convert_to_preference_format(
        dataset,
        negative_generator,
    ...
    preference_dataset = filter_identical_pairs(preference_dataset)
    
    # Print sample examples
    verify_and_print_samples(preference_dataset, n_samples=args.print_samples)
    
    # Save
    print(f"\n{'='*80}")
    print(f"SAVING PREFERENCE DATASET")
    print(f"{'='*80}")
    print(f"Output directory: {args.output_dir}")
    
    Path(args.output_dir).mkdir(parents=True, exist_ok=True)
    preference_dataset.save_to_disk(args.output_dir)
    print("✓ Dataset saved")
    ...
    with open(os.path.join(args.output_dir, "preference_config.json"), "w") as f:
        json.dump(config, f, indent=2)
    
    print("✓ Configuration saved")
    
    print("\n" + "="*80)
    print("✅ TARGETED PREFERENCE DATASET PREPARATION COMPLETE")
    print("="*80)
    print(f"\nDataset statistics:")
    print(f"  Original samples: {len(dataset)}")
    print(f"  Preference pairs generated: {len(preference_dataset)}")
    print(f"  Negatives per sample: {args.negatives_per_sample}")
    print(f"  Quality: ALL pairs verified different ✓")
    print(f"\nNext step:")
    print(f"  python train_preference.py --train_data_path {args.output_dir}")
