#!/usr/bin/env python3
"""
Token-weighted preference training (chosen/rejected) with:
...
# Data collator
# ============================================================================
class PreferenceDataCollatorTokenWeighted:
    """
...
# Trainer
# ============================================================================
class TWCPreferenceTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # Preference logits and loss
        pi_logratio = pi_c - pi_r
        ref_logratio = ref_c - ref_r
        pref_logits = pi_logratio - ref_logratio
        p_pref = torch.sigmoid(self.cfg.beta * pref_logits)
        preference_loss = -F.logsigmoid(self.cfg.beta * pref_logits)  # [B]
        ...
        total = preference_loss.mean() + barrier_loss
        ...
        with torch.no_grad():
            ...
            metrics = {
                ...
                "loss/preference": preference_loss.mean().detach().float(),
                ...
                "logits/mean": pref_logits.mean().detach().float(),
                "logits/max": pref_logits.max().detach().float(),
                "logits/min": pref_logits.min().detach().float(),
            }
...
def main():
    ...
    print("=" * 80)
    print("Token-Weighted Preference Training with Adaptive Barrier (FIXED)")
    print("=" * 80)
    ...
    collator = PreferenceDataCollatorTokenWeighted(tok, cfg=cfg, system_prompt=args.system_prompt)
